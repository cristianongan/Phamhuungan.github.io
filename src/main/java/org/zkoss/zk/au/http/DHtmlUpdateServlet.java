 ...
 * Though it is called DHtmlUpdateServlet, it is used to serve all kind of
 * HTTP-based clients, including ajax (HTML+Ajax), mil (Mobile Interactive
 * Language), and others (see {@link Desktop#getDeviceType}.
 *
 * <p>
 * Init parameters:
 *
 * <dl>
 * <dt>compress</dt>
 * <dd>It specifies whether to compress the output if the browser supports the
 * compression (Accept-Encoding).</dd>
 * <dt>extension0, extension1...</dt>
 * <dd>It specifies an AU extension ({@link AuExtension}). The
 * <code>extension0</code> parameter specifies the first AU extension, the
 * <code>extension1</code> parameter the second AU extension, and so on.<br/>
 * The syntax of the value is<br/>
 * <code>/prefix=class</code></dd>
 * </dl>
 *
 * <p>
 * By default: there are two extensions are associated with "/upload" and
 * "/view" (see {@link #addAuExtension}. Also, "/web" is reserved. Don't
...
public class DHtmlUpdateServlet extends HttpServlet {
	 ...
	 * {@link DHtmlUpdateServlet} is started.
	 * <p>
	 * Unlike {@link #addAuExtension(String, AuExtension)}, it can be called even if
	 * the update servlet is not loaded yet ({@link #getUpdateServlet} returns
	 * null).
	 *
	 * <p>
	 * If there was an AU extension associated with the same name, the the old AU
	...
	/**
	 * Returns the update servlet of the specified application, or null if not
	 * loaded yet. Note: if the update servlet is not loaded, it returns null.
	 * 
	 * @since 3.0.2
	 */
	 ...
	 *
	 * <p>
	 * If you want to add an Au extension, even before DHtmlUpdateServlet is
	 * started, use {@link #addAuExtension(WebApp, String, AuExtension)} instead.
	 *
	 * @param prefix    the prefix. It must start with "/", but it cannot be "/" nor
	 *                  "/web" (which are reserved).
	 * @param extension the AU extension (never null).
	 * @return the previous AU extension associated with the specified prefix, or
	 *         null if the prefix was not associated before.
...
//		if (log.finerable()) log.finer("Path info: "+pi);
	...
	/**
	 * Returns the AU extension associated with the specified prefix, or null if no
	 * AU extension associated.
	 * 
	 * @since 5.0.0
	 */
	...
	/**
	 * Returns the first AU extension matches the specified path, or null if not
	 * found.
	 */
	...
	/**
	 * Returns the desktop of the specified ID, or null if not found. If null is
	 * returned, {@link #recoverDesktop} will be invoked.
	 * 
	...
	protected void process(Session sess, HttpServletRequest request, HttpServletResponse response)
		...
		// final String needRedirect = response.getHeader("VSA-Redirect");//Linhntn
		final String needRedirect = (String) request.getAttribute("VSA-Redirect");// LongH edit
		if (errClient != null) {
			if (log.debugable()) {
...
//		if (log.debugable()) log.debug("AU request: "+aureqs);
	...
	/**
	 * Recovers the desktop if possible. It is called if {@link #getDesktop} returns
	 * null.
	 * <p>
	 * The default implementation will look for any failover manager
	 * ({@link FailoverManager}) is registered, and forward the invocation to it if
	 * found.
	 * 
	...
	private void sessionTimeout(HttpServletRequest request, HttpServletResponse response, WebApp wapp, String dtid)
		...
		if (!getAuDecoder(wapp).isIgnorable(request, wapp)) {
			...
			// String uri = response.getHeader("VSA-Location");//ui != null ? ui.uri:
			// null;Linhntn - LongH edit
//            String uri = (String) request.getAttribute("VSA-Location");
