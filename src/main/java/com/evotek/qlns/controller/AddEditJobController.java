 ...
 */
@Controller
public class AddEditJobController extends BasicController<Window> implements Serializable {

	private static final long serialVersionUID = -2598959786950022235L;

	private static final Logger _log = LogManager.getLogger(AddEditJobController.class);
	...
	private boolean _validate(String jobTitle) {
		if (Validator.isNull(jobTitle)) {
			this.tbJobTitle.setErrorMessage(Values.getRequiredInputMsg(Labels.getLabel(LanguageKeys.JOB_TITLE)));
		...
		if (jobTitle.length() > Values.MEDIUM_LENGTH) {
			this.tbJobTitle.setErrorMessage(
					Values.getMaxLengthInvalidMsg(Labels.getLabel(LanguageKeys.JOB_TITLE), Values.MEDIUM_LENGTH));
	...
	public void onClick$btnAdd() throws Exception {
		try {
			String jobTitle = GetterUtil.getString(this.tbJobTitle.getValue());
