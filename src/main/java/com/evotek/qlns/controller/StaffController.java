...
@Controller
@Scope("prototype")
public class StaffController extends BasicController<Hlayout> implements Serializable {
	private static final long serialVersionUID = 3518744289421228673L;
...
//        listboxResult.invalidate();
	...
	public void advanceSearch() {
		...
		ListModel model = new StaffListModel(this.listboxResult.getPageSize(), staffName, yearOfBirth, dept, email, job,
				phone, this.isAdvance, false, null, this.staffService);

		this.listboxResult.setModel(model);

		this.listboxResult.setItemRenderer(new StaffRender(this.winStaff, model));
	...
	public void basicSearch() {
		...
		ListModel model = new StaffListModel(this.listboxResult.getPageSize(), keyword, this.isAdvance, false, null,
				this.staffService);
	...
	private void createDeptNav(Department dept, Component parentNode) {
		List<Department> childs = this.departmentService.getDepartmentByParentId(dept.getDeptId());
		...
		if (Validator.isNull(childs)) {
			...
			navItem.addForward(Events.ON_CLICK, this.winStaff, "onSelectDept", new Object[] { navItem, dept });
			...
			nav.addForward(Events.ON_OPEN, this.winStaff, "onSelectDept", new Object[] { nav, dept });
	...
	private List<Long> getListDeptChild(Department deptParent, List<Long> deptIds) {
	...
	public void initData() {
		this.firstAhGroupPage = new Auxheader[] { this.ahGeneral };
		this.secondAhGroupPage = new Auxheader[] { this.ahContract, this.ahInsurance };
		this.thirdAhGroupPage = new Auxheader[] { this.ahQualifications, this.ahIdentification, this.ahContact };

		this.firstLhGroup = new Listheader[] { this.lhDepartmentName, this.lhJobTitleName, this.lhWorkDate,
				this.lhDateOfBirth, this.lhPermanentResidence, this.lhCurrentResidence, this.lhStatus, this.lhNote };
		this.secondLhGroup = new Listheader[] { this.lhContractType, this.lhContractFromDate, this.lhContractToDate,
		...
		this.thirdLhGroup = new Listheader[] { this.lhLevels, this.lhMajors, this.lhCollege, this.lhIdentityCard,
	...
	public void onClick$btnNextPage() {
		int _page = GetterUtil.getIntegerValue(this.listboxResult.getAttribute("page"), 1);
	...
	public void onSelectDept(Event event) {
		...
		ListModel model = new StaffListModel(this.listboxResult.getPageSize(), StringPool.BLANK, true, true, deptIds,
				this.staffService);
