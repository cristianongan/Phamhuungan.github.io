...
public class GetterUtil {
    ...
    public static String getStatusColorName(Long status,
            String[] statusColorArray) {
        String statusName = StringPool.BLANK;

        if (status != null && status > 0
                && status < statusColorArray.length) {
    ...
    public static String[] getLanguageValue(String[] keys){
        String[] labels = new String[keys.length];

        for(int i=0; i<keys.length; i++){
            labels[i] = Labels.getLabel(keys[i]);
        }
