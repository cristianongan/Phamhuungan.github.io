...
public class UserRender implements ListitemRenderer<User> {
	...
	public void render(Listitem items, User user, int index) throws Exception {
        ...
        items.appendChild(new Listcell(_getPhoneNumber(user.getPhone(),
                user.getMobile())));
...
//                Constants.STYLE_TEXT_ALIGN_CENTER));
    ...
    private Listcell _getAction(User user) {
        ...
        hlayout.appendChild(ComponentUtil.createButton(null,
                ...
                _createParameterMap(user), Constants.Z_ICON_PENCIL, 
                Constants.BLUE));
        ...
        hlayout.appendChild(ComponentUtil.createButton(null,
                Labels.getLabel(LanguageKeys.ASSIGN_ROLE), 
                ComponentUtil.ASSIGN_ROLE_TOOLTIP, Events.ON_CLICK,
                ASSIGNED_ROLE_PAGE, _createParameterMap(user),
    ...
    private Listcell _getGender(Long gender) {
        String[] genderTypeArray = new String[]{
            Labels.getLabel(LanguageKeys.MALE),
            Labels.getLabel(LanguageKeys.FEMALE)
        };

        Listcell genderName = new Listcell(
                GetterUtil.getStatusName(gender, genderTypeArray));
    ...
    private static final String ASSIGNED_ROLE_PAGE =
            "/html/pages/manager_user/assigned_role.zul";
}
