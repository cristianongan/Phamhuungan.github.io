...
public class ContractTypeRender implements ListitemRenderer<ContractType>{
    ...
    private void renderEditing(Listitem item, ContractType ct, int index) {
        ...
        item.appendChild(ComponentUtil.createSpinnerListcell(ct.getMonthDuration(), 
                ct.getContractTypeId()+"-"+"month-duration", 
                Constants.STYLE_TEXT_ALIGN_CENTER));
    ...
    private void renderNoneEditing(Listitem item, ContractType ct, int index) {
        ...
        if(ct.getMonthDuration() == null) {
            ...
            item.appendChild(ComponentUtil.createListcell(
                GetterUtil.getString(ct.getMonthDuration())));
        }
        
        item.appendChild(_getNoneEditAction(item, ct, index));
    }
    
    private Listcell _getNoneEditAction(final Listitem item, final ContractType ct, 
            final int index) {
        ...
        Button btnEdit = ComponentUtil.createButton(
    ...
    private Listcell _getEditAction(final Listitem item, final ContractType ct, 
            final int index) {
        ...
        Button btnSave = ComponentUtil.createButton(
                Labels.getLabel(LanguageKeys.BUTTON_SAVE), Constants.Z_ICON_SAVE, 
                Constants.BLUE);
        ...
        btnSave.addEventListener(Events.ON_CLICK, new EventListener<Event>() {
            ...
            public void onEvent(Event t) throws Exception {
                ...
                if(cmp1 instanceof Textbox){
                    ct.setContractTypeName(GetterUtil.getString(
                            ((Textbox) cmp1).getValue()));
        ...
        Button btnCancel = ComponentUtil.createButton(
                Labels.getLabel(LanguageKeys.BUTTON_CANCEL), Constants.Z_ICON_TIMES, 
                Constants.RED);
