 ...
 * @author hungnt81
 */
public class TreeCategoryRender<Component> implements
        TreeitemRenderer<CategoryTreeNode> {
...
//        dataRow.appendChild(new Treecell());
        ...
        dataRow.appendChild(ComponentUtil.createTreeCell(
                _getType(category.getType())));//Loai menu
        dataRow.appendChild(ComponentUtil.createTreeCell(
                category.getUserName()));// Nguoi tao
        dataRow.appendChild(ComponentUtil.createTreeCell(
                DateUtil.formatLongDate(category.getModifiedDate())));
        dataRow.appendChild(ComponentUtil.createTreeCell(
                Values.getLockStatus(category.getStatus())));
    ...
    private String _getType(Long categoryType){
        ...
        if(Validator.isNotNull(categoryType)
                && categoryType.intValue()< _type.length){
    ...
    private Treecell _getAction(Category category){
        ...
        hlayout.appendChild(ComponentUtil.createButton(null,
                ...
                _createParameterMap(category), Constants.Z_ICON_PENCIL, 
                Constants.BLUE));
        ...
        if(Values.MENU_TYPE_ITEM.equals(category.getType())){
            hlayout.appendChild(ComponentUtil.createButton(null,
                Labels.getLabel(LanguageKeys.BUTTON_RIGHT), 
                ComponentUtil.RIGHT_TOOLTIP, Events.ON_CLICK, RIGHT_PAGE,
                _createParameterMap(category), Constants.Z_ICON_KEY, 
                Constants.ORANGE));
...
//                Constants.Z_ICON_KEY, Constants.ORANGE));
    ...
    private static final String RIGHT_PAGE =
            "/html/pages/manager_menu/right.zul";

    private static final String RESOURCE_ACTION_PAGE =
            "/html/pages/manager_menu/resource_action.zul";
