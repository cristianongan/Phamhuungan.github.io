...
public class StaffRender implements ListitemRenderer<Staff> {
    ...
    public void render(Listitem item, Staff staff, int index) throws Exception {
        ...
        item.appendChild(ComponentUtil.createListcell(GetterUtil.getDate(
                ...
                Constants.STYLE_TEXT_ALIGN_CENTER));
        item.appendChild(ComponentUtil.createListcell(
                staff.getPermanentResidence()));
        item.appendChild(ComponentUtil.createListcell(
                staff.getCurrentResidence()));
        item.appendChild(ComponentUtil.createListcell(
                Values.getStaffStatus(staff.getStatus())));
        item.appendChild(ComponentUtil.createListcell(staff.getNote()));
        item.appendChild(ComponentUtil.createListcell(
                staff.getContractType().getContractTypeName()));
        item.appendChild(ComponentUtil.createListcell(GetterUtil.getDate(
        ...
        item.appendChild(ComponentUtil.createListcell(GetterUtil.getFormat(
                staff.getSalaryBasic()), Constants.STYLE_TEXT_ALIGN_CENTER));
        item.appendChild(ComponentUtil.createListcell(GetterUtil.getDate(
        ...
        item.appendChild(new Listcell(_getPhoneNumber(staff.getMobile(),
                staff.getHomePhone())));
...
//                ? "" : staff.getChildNumber().toString()));
    ...
    private Listcell _getAction(Staff staff, int index) {
        ...
        hlayout.appendChild(ComponentUtil.createButton(null,
                ...
                Events.ON_CLICK, EDIT_PAGE,
                _createParameterMap(staff, Labels.getLabel(
                LanguageKeys.TITLE_EDIT_STAFF), index), Constants.Z_ICON_PENCIL, 
                Constants.BLUE));
        ...
        hlayout.appendChild(ComponentUtil.createButton(null,
                Labels.getLabel(LanguageKeys.DETAIL), ComponentUtil.DETAIL_TOOLTIP,
                Events.ON_CLICK, DETAIL_PAGE, _createParameterMap(staff, Labels.getLabel(
                LanguageKeys.TITLE_STAFF_DETAIL), index), Constants.Z_ICON_SEARCH_PLUS, 
                Constants.PURPLE));
    ...
    private static final String DETAIL_PAGE =
            "/html/pages/manager_human_resource/detail.zul";
}
