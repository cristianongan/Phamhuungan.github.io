...
import com.viettel.automl.service.IDateService;
import com.viettel.automl.service.IJWTService;
...
public class JWTService implements IJWTService, Clock {
	...
	@Autowired
	JWTService(final IDateService dates, @Value("${jwt.issuer:1}") final String issuer,
			@Value("${jwt.expiration-sec:1}") final int expirationSec,
			@Value("${jwt.clock-skew-sec:1}") final int clockSkewSec, @Value("${jwt.secret:1}") final String secret) {
		super();
