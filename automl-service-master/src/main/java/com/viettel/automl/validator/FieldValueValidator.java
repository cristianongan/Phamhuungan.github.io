package com.viettel.automl.validator;
...
public class FieldValueValidator implements ConstraintValidator<FieldValue, Object> {
	...
	private boolean validateField(Object value) {
		...
		if (value instanceof Integer) {
			...
			return Arrays.stream(constraintAnnotation.numbers()).anyMatch(v -> Objects.equals(v, typedField));
		}
		if (value instanceof String) {
			String typedField = (String) value;
			return Arrays.stream(constraintAnnotation.strings()).anyMatch(v -> v.equalsIgnoreCase(typedField));
		}
		return false;
	}
}
